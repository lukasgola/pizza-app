...
function EditNotes({ navigation }) {
    ...
    const [deliveryTown, setDeliveryTown] = useState(item.note.deliveryTown)
    const [deliveryStreet, setDeliveryStreet] = useState(item.note.deliveryStreet)
    const [deliveryHours, setDeliveryHours] = useState(item.note.deliveryHours)
    const [deliveryMin, setDeliveryMin] = useState(item.note.deliveryMin)

    const [pizzaName, setPizzaName] = useState(item.note.pizzaName)
    const [pizzaSize, setPizzaSize] = useState(item.note.pizzaSize)
    const [pizzaPrice, setPizzaPrice] = useState(item.note.pizzaPrice)
    ...
    const [sauceName, setSauceName] = useState(item.note.sauceName)
    const [saucePrice, setSaucePrice] = useState(item.note.saucePrice)
    ...
    var orderHours = item.note.orderHours;
    var orderMin = item.note.orderMin;

    function onSaveNote() {
        navigation.state.params.deleteNote(item.id)
        navigation.state.params.editNote(item.id,{ deliveryTown, deliveryStreet, deliveryHours, deliveryMin, pizzaName, pizzaSize, pizzaPrice, sauceName, saucePrice, suma, orderHours, orderMin })
        navigation.goBack()
    }
            ...
            <View style={styles.container}>
                ...
                <View style={styles.food}>
                    <TouchableOpacity
                        ...
                        onPress={() => navigation.navigate('EditOrder', {addPizza})}
                    >
                        <Image style={styles.image} source={require('../../assets/icons/pizza.png')} />
                    </TouchableOpacity>
                    <TouchableOpacity
                        style={styles.menuOption}
                        onPress={() => navigation.navigate('EditSauceOrder', {addSauce})}
                    >
